...
- 도구·분류기: 마크업은 그대로, `body.pb-v2` 층(`pibo-ui.css` 맨 아래)이 색·모양만 바꾼다
- v2 IDE 구조: 상단바 44px(파일 이름·저장 | 블록/파이썬 | 실행·정지 | 도구·대화·분류기 | 더보기),
  왼쪽 레일(파일·터미널 여닫기, 보기 = 글자 크기·편집기 테마), 어두운 터미널 + 아래 입력줄.
  초기화·전원·도움말·전체화면·언어는 [더보기] 안
- 검증: 기존 IX 27항목을 v2 주소로 돌려 전부 통과, v2 전용 IX 17항목(레일·메뉴·스테퍼·
  테마·언어·대화상자·WiFi), 560~1960px 한/영 상단바 넘침 0
