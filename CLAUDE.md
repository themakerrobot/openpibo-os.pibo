...
같은 기기(Pi 4 · 커널 6.6.62 · 펌웨어 7.45.265)에서 `iw reg set` 으로 바꿔가며 실측했다.
재부팅 없이 몇 초 사이에 재현되므로 하드웨어·펌웨어·`brcmfmac.conf` 는 전부 무관하다.

```bash
sudo iw reg set KR; sleep 3; sudo iw phy0 info | grep -E "5745|5785|5825"   # 20.0 dBm
sudo iw reg set PH; sleep 3; sudo iw phy0 info | grep -E "5745|5785|5825"   # disabled
```
...
규칙이 있는데 적용이 안 된다. **메커니즘 미확인** — `wireless-regdb` / 커널 쪽으로 의심하고
있고, regdb 업데이트로 풀리는지 확인이 남아 있다. 추측으로 단정하지 말 것.
...
| 항목 | 값 | 이유 |
|---|---|---|
| 5GHz 채널 | **PH 배포판은 36~48 중 하나로 고정**<br>KR 은 149~165 도 가능 | PH 는 상위 채널이 막혀 있다. `자동` 은 DFS·상위로 옮겨간다 |
...
**무선 확장(리피터)은 백홀 밴드가 서비스 채널을 결정한다.**
