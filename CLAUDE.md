...
로봇이 쓸 수 있는 **5GHz 채널은 36 / 40 / 44 / 48 뿐이다.** `sudo iw phy0 info` 실측:

```
5180 [36] 5200 [40] 5220 [44] 5240 [48]   17.0 dBm              ← 사용 가능
5260 ~ 5700 [52~140]                      no IR, radar detection ← DFS
5745 ~ 5825 [149~165]                     disabled               ← 완전 차단
```

`brcmfmac` 이 wiphy 에 거는 world 도메인(`phy#0 country 99`) 때문이고,
`cfg80211.ieee80211_regdom` 값과 무관하다. **국내·필리핀 동일하다.**

공유기 설정 규칙:

| 항목 | 값 | 이유 |
|---|---|---|
| 5GHz 채널 | **36~48 중 하나로 고정** | `자동` 은 149~165·DFS 로 옮겨간다 |
| 채널 크기 | **80 이하** | 채널 36 + 160MHz 는 5170~5330 을 써서 DFS(52~64)를 끌어들인다 |
| 모드 | **11ac** | CYW43455 는 11ax 미지원 |

**무선 확장(리피터)은 백홀 밴드가 서비스 채널을 결정한다.**
5GHz 백홀이면 확장기 5GHz가 상위 채널에 묶인다 — 상위가 157이면 로봇이 전부 떨어진다.
