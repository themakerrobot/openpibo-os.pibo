...
- v2 IDE 구조 — **가로 줄은 상단바 하나뿐**이다. 패널 제목줄을 두지 않는다(레일이 이름표).
  ...
  - 파일 패널 첫 줄: 경로 + [추가](새 파일·새 폴더·업로드). 미리보기는 사진·소리를 열 때만 뜬다
...
- soft: 순백(#fff) 대신 옅은 회색(패널 #f5f5f2, 상단바 #efefeb). 글자는 #1d2126 로 대비 약 15:1,
  보조 글자도 7:1 이상. 흰 바탕을 L*≈96 로 낮추는 게 핵심이고 대비는 거의 안 줄었다
