...
**아직 시안이다. 기본은 v1(`index.html`).** 주소 끝에 `?ui=v2` 를 붙여 켜고 `?ui=v1` 로 끈다.

- 고른 값은 쿠키 `pibo_ui` 에 남는다(`design/pibo-ui.js`). 쿠키는 포트를 안 가려서
  IDE(80)에서 고르면 도구(50000)·분류기(50010)도 따라간다
- IDE: `run_ide.py` 의 `/` 가 쿼리 → 쿠키 순으로 보고 `templates/index_v2.html` 을 준다.
...
- 도구·분류기: 마크업은 거의 그대로, `body.pb-v2` 층(`pibo-ui.css` 맨 아래)이 색·모양을 바꾼다.
  도구 왼쪽 메뉴에만 라벨(`.pb-navlabel`)을 넣었다(v1 에서는 숨김)
- v2 IDE 구조 — **가로 줄은 상단바 하나뿐**이다. 패널 제목줄을 두지 않는다(레일이 이름표).
  - 상단바 44px: 파일 이름·저장 | 블록/파이썬 | 실행·정지 | 도구·대화·분류기 | 더보기
    (초기화·전원·도움말·전체화면·언어는 [더보기] 안)
  - 파일 패널 첫 줄: 경로 + [추가](새 파일·새 폴더·업로드). 아래 미리보기(사진·소리)는 늘 있고
    머리를 누르면 접힌다(기억). 파일을 열면 접혀 있어도 펴지고, 사진을 누르면 크게 본다
  - 터미널: 패널 전체가 콘솔. 경로·실행 상태·지우기가 콘솔 안 머리줄에 있다
  - **화면 출력**: `[IDE에 보기]` 블록(`camera.imshow_to_ide` → `POST /show` → `update{image, filepath:'/home/pi/.tmp.jpg'}`)
    의 그림은 파일 미리보기가 아니라 터미널 위 '화면 출력' 칸에 뜬다(실행 결과라서). 파일 패널이 접혀 있어도
    보이고, 터미널이 접혀 있으면 편다. 누르면 크게 보고, 크게 보는 동안에도 프레임이 바뀐다.
    닫으면 그 실행 동안은 안 뜨고 다음 실행에서 다시 뜬다. 구분은 경로 `/home/pi/.tmp.jpg` 로 한다
    (`ide/static/v2/ide.js` 의 `LIVE_PATH`) — `run_ide.py` 의 `/show` 경로를 바꾸면 같이 바꿀 것.
    v1 은 예전처럼 파일 미리보기 칸에 뜬다
  - 왼쪽 레일: 파일·터미널 여닫기, [보기] = 화면 밝기·글자 크기·편집기 테마
...
- **색 조합 Ink & Amber** — 틀 먹빛 남색 #1c2130, 주 동작 앰버 #f7b924(로고 마크·[실행]·지금 연 파일),
  상태 인디고 #4c59d6(선택·포커스·스위치·슬라이더). 앰버와 인디고는 보색이라 서로 부딪치지 않는다.
  전엔 노랑·청록·빨강·Blockly 색이 한꺼번에 떠서 장난감 같았다. CSS 변수 이름의 `yellow` 는 앰버, `accent` 는 인디고다
- **틀 색 시안 두 가지** — 먹빛(기본) / 청록(`?frame=teal` 로 켜고 `?frame=ink` 로 끈다, 쿠키 `pibo_frame`,
  `html[data-frame="teal"]`). 청록안은 Arduino IDE 처럼 틀을 브랜드 색(파이보 청록을 깊게 한 #0f4c55)으로 하고
  상태색도 청록으로 묶어 '청록 + 앰버' 두 색만 쓴다. 정하면 한쪽을 지운다
- **틀(상단바·레일·상태바, 도구·분류기 헤더·왼쪽 메뉴)은 세 밝기 모두 같은 색.**
...
v2 전용 IX 25항목(레일·메뉴·추가·미리보기·밝기·스테퍼·테마·언어·대화상자·WiFi), 기존 IX 27항목을 v2 주소로,
560~1960px 한/영 상단바 넘침 0, 도구 메뉴 3개 전환, 분류기 학습·저장 e2e, 세 밝기 × 세 앱 pageerror 0.
**실기기·실제 노트북 패널로는 아직 안 봤다.**

채택하면: `index.html` 을 v2 로 바꾸고 `index.css`·`body.pb-refresh`·`pb-v2` 층을 정리한다.
그 전까지 두 벌이 공존한다
