...
파이보 카메라를 태블릿 브라우저로 보내고, 브라우저가 학습한 모델을 파이보에 저장한다.

  카메라   파이보 → 브라우저: 320x240 JPEG (socket.io 'camera_image')
  학습     브라우저 (MediaPipe wasm 으로 특징을 뽑고 TF.js 로 작은 분류기를 학습)
  저장     브라우저 → 파이보: /home/pi/mymodel/<모델 이름>/
  추론     파이보에서 openpibo.vision_classify.CustomClassifier (TensorFlow 없음)
