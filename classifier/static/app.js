// ═══════════════════════════════════════════════════════════
// 분류기 — 파이보 카메라로 이미지 · 손 · 얼굴 · 포즈를 가르친다
// ═══════════════════════════════════════════════════════════
// 흐름은 Teach Lab 과 같다: 종류 만들기 → 예시 모으기 → 배우기 → 시험 → 저장.
// 다른 점은 카메라가 태블릿이 아니라 **파이보** 라는 것 하나다.
//
//   파이보 카메라 ─(socket.io, 320x240 JPEG)→ 이 페이지 <img>
//   <img> → MediaPipe(wasm) 로 특징 뽑기 → TF.js 로 작은 분류기 학습
//   저장 → POST /api/models → /home/pi/mymodel/<이름>/
//   파이보 블록 → openpibo.vision_classify.CustomClassifier 가 같은 계산으로 추론
//
...
// 파이보가 보낸 그림. 처리 중이면 가장 최근 것 하나만 기다리게 한다
